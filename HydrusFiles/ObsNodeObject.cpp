...
ObsNodeObject::ObsNodeObject(int gid, pqxx::connection &qry, HydrusParameterFilesManager *parent)
{
    _parent = parent;
    _NS = _parent->NumofSolute();
    _NObs = _parent->NumofObsNodes();
...
bool ObsNodeObject::Save(const std::string &path)
{
    ...
    std::ofstream out((std::filesystem::absolute(p) / "Obs_Node.out").string());
    if (!out)
    {
        return false;
    }
...
bool ObsNodeObject::Save(std::ostream &out)
{
    if (!_isValid || !out)
    {
        return false;
...
bool ObsNodeObject::open(const std::string &filename)
{
    ...
    if (_tmCnt)
    {
        _time = std::make_unique<double[]>(_tmCnt);
        _head = std::make_unique<double[]>(_tmCnt * _NObs);
        _theta = std::make_unique<double[]>(_tmCnt * _NObs);
        _flux = std::make_unique<double[]>(_tmCnt * _NObs);
        if (_NS)
        {
            _conc = std::make_unique<double[]>(_tmCnt * _NObs * _NS);
        }
    }
    else
    {
...
bool ObsNodeObject::open(int gid, pqxx::connection &qry)
{
    try
    {
        pqxx::work w(qry);
        std::string sql = "select count(distinct tm) from obs_node where gid=$1;";
        pqxx::row r = w.exec_params1(sql, gid);
        w.commit();
        _tmCnt = r[0].as<int>();
    }
    ...
    strbld << " from obs_node where gid=" << gid << " order by tm,obsid;";
    int i = 0;
    int cnt = 0;
    int idx = 0, idx2 = 0;
    try
    {
        ...
        for (auto it = r.begin(); it != r.end(); ++it)
        {
            if (cnt == 0)
            {
                idx = i * _NObs;
                idx2 = i * _NObs * _NS;
                _time[i] = it[0].as<double>();
                i++;
            }
            _head[idx + cnt] = it[1].as<double>();
            _theta[idx + cnt] = it[2].as<double>();
            _flux[idx + cnt] = it[3].as<double>();
            //int id=it[4].as<int>();
            int k = idx2 + cnt * _NS;
            for (int j = 0; j < _NS; ++j)
            {
                _conc[k + j] = it[4 + j].as<double>();
            }
            cnt++;
            if (cnt == _NObs)
            {
