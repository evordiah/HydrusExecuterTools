...
NodInfoObject::NodInfoObject(int gid, pqxx::connection &qry, HydrusParameterFilesManager *parent)
{
    _parent = parent;
...
bool NodInfoObject::Save(const std::string &path)
{
    ...
    std::ofstream out((std::filesystem::absolute(p) / "Nod_Inf.out").string());
    if (!out)
    {
        return false;
    }
...
bool NodInfoObject::Save(std::ostream &out)
{
    if (!_isValid || !out)
    {
        return false;
...
bool NodInfoObject::open(const std::string &filename)
{
    ...
    while (std::getline(in, line))
    {
        ...
        while (std::getline(in, line))
        {
            ...
            pRec = std::make_unique<NodinfoRecord>(line.c_str(), NS);
...
bool NodInfoObject::open(int gid, pqxx::connection &qry)
{
    ...
    try
    {
        ...
        for (auto it = r.begin(); it != r.end(); ++it)
        {
            ...
            pRec = std::make_unique<NodinfoRecord>(it, NS);
            _Recs[tm].push_back(std::move(pRec));
        }
    }
...
void NodInfoObject::WriteSection(std::ostream &out, double time, std::list<std::unique_ptr<NodInfoObject::NodinfoRecord>> &lst, const int NS)
{
    ...
    {
        out << " Node      Depth      Head Moisture       K          C         "
               "Flux        Sink         Kappa   v/KsTop   Temp   Conc(1..NS) Sorb(1...NS)"
            << std::endl;
        out << "           [L]        [L]    [-]        [L/T]      [1/L]      ["
               "L/T]        [1/T]         [-]      [-]      [C]      [M/L*3]"
            << std::endl;
...
NodInfoObject::NodinfoRecord::NodinfoRecord(const char *pline, const int NS)
{
    ...
    if (NS)
    {
        _Conc = std::make_unique<double[]>(NS);
        _Sorb = std::make_unique<double[]>(NS);
        for (int i = 0; i < NS; ++i)
        {
...
NodInfoObject::NodinfoRecord::NodinfoRecord(pqxx::row &row, const int NS)
{
    Node = row[1].as<int>();
    Depth = row[2].as<double>();
    Head = row[3].as<double>();
    Moisture = row[4].as<double>();
    K = row[5].as<double>();
    C = row[6].as<double>();
    Flux = row[7].as<double>();
    Sink = row[8].as<double>();
    Kappa = row[9].as<int>();
    vdivKsTop = row[10].as<double>();
    Temp = row[11].as<double>();
    if (NS)
    {
        _Conc = std::make_unique<double[]>(NS);
        _Sorb = std::make_unique<double[]>(NS);
        for (int i = 0; i < NS; ++i)
        {
            _Conc[i] = row[12 + i].as<double>();
        }
        for (int i = 0; i < NS; ++i)
        {
            _Sorb[i] = row[12 + NS + i].as<double>();
        }
    }
}
