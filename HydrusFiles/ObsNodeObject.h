...
public:
    ...
    ObsNodeObject(int gid, QSqlQuery &qry,HydrusParameterFilesManager *parent);
    operator bool()
    {
        return _isValid;
    }
...
private:
    ...
    bool ParseLine(const char* p,const int nsize,const int lineindex);
    void SaveLine(std::ostream&out,const int lineindex);
    std::string LineToSql(const int gid,const int lineindex);
private:
    bool _isValid;
    int _NObs;
    int _NS;
