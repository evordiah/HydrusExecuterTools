...
TLevelObject::TLevelObject(int gid, QSqlQuery &qry, HydrusParameterFilesManager *parent)
{   
    _parent=parent;
...
bool TLevelObject::Save(const std::string &path)
{
    ...
    p=dir.absoluteFilePath("T_Level.out");
...
std::string TLevelObject::ToSqlStatement(const int gid)
{
    ...
    QString s=QString("update selector set iday=%1,imonth=%2,ihours=%3,imins=%4,isecs=%5"
                      " where gid=%6 ;").arg(_iday).arg(_imonth).arg(_ihours).arg(_imins).arg(_isecs).arg(gid);
    out<<s.toStdString();
...
bool TLevelObject::open(const std::string &filename)
{
    ...
    pRec.reset(new TLevelRecord(line.c_str()));
...
bool TLevelObject::open(int gid, QSqlQuery &qry)
{
    QString sql=QString("select tm, rtop, rroot, vtop, vroot, vbot, sr_top, sr_root, "
                        "sv_top, sv_root, sv_bot, htop, hroot, hbot, runoff, s_runoff, "
                        "volume, s_infil, s_evap, tlevel, s_wtrans, snowlayer "
                        "from t_level where gid=%1 order by tm;").arg(gid);
    if(!qry.exec(sql))
    {
        return false;
    }
    ...
    while(qry.next())
    {
        pRec.reset(new TLevelRecord(qry));
        _Recs.push_back(std::move(pRec));
    }
    return true;
}
...
TLevelObject::TLevelRecord::TLevelRecord(QSqlQuery &qry)
{
    Time=qry.value(0).toDouble();
    rTop=qry.value(1).toDouble();
    rRoot=qry.value(2).toDouble();
    vTop=qry.value(3).toDouble();
    vRoot=qry.value(4).toDouble();
    vBot=qry.value(5).toDouble();
    sum_rTop=qry.value(6).toDouble();
    sum_rRoot=qry.value(7).toDouble();
    sum_vTop=qry.value(8).toDouble();
    sum_vRoot=qry.value(9).toDouble();
    sum_vBot=qry.value(10).toDouble();
    hTop=qry.value(11).toDouble();
    hRoot=qry.value(12).toDouble();
    hBot=qry.value(13).toDouble();
    RunOff=qry.value(14).toDouble();
    sum_RunOff=qry.value(15).toDouble();
    Volume=qry.value(16).toDouble();
    sum_Infil=qry.value(17).toDouble();
    sum_Evap=qry.value(18).toDouble();
    TLevel=qry.value(19).toInt();
    Sum_WTrans=qry.value(20).toDouble();
    SnowLayer=qry.value(21).toDouble();
}
