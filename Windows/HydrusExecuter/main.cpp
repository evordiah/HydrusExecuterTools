                  ...
                  std::string& strexepath,int& threadcnt,QSqlDatabase& db,bool& bEven)
{
	...
	std::string logfile = QDir::toNativeSeparators(currentpath.absoluteFilePath("hydrusexecutingerrlog.log")).toStdString();
    ValueArg<std::string> logarg("","log","the file for recording errors ",false,logfile,"string");
    cmd.add(logarg);
    ...
    ValueArg<std::string> hydrusexearg("e","hydrus_path","set executable hydrus file to run",true,"","string");
...
{
    using namespace std;
	QCoreApplication nQCoreApp(argc,argv);
